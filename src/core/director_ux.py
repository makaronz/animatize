    ...
    def get_preset(preset_type: StylePreset) -> DirectorControls:
        ...
        if preset_type == StylePreset.DOCUMENTARY:
            return DirectorControls(
                ...
                motion=MotionControl(
                    overall_strength=MotionStrengthLevel.SUBTLE,
                    subject_motion=0.4,
                    camera_motion=0.6
                ),
            ...
            return DirectorControls(
                mode=DirectorMode.PRO,
                camera=CameraControl(
                    movement_type=CameraMovementType.TRACK,
            ...
            return DirectorControls(
                mode=DirectorMode.PRO,
                camera=CameraControl(
                    movement_type=CameraMovementType.PUSH,
                    angle=CameraAngle.EYE_LEVEL,
...
class IterationWorkflow:
    ...
    def compare_generations(
        ...
        # Sort by rating
        sorted_gens = sorted(
            [g for g in generations if g.rating is not None],
...
if __name__ == "__main__":
    ...
    variation = workflow.create_variation({
        "camera.speed": 1.2,
        "motion.overall_strength": MotionStrengthLevel.STRONG.value
    })
